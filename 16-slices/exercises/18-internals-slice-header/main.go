...
func main() {
	...
	printSize("Array", unsafe.Sizeof(arr))
	printSize("Array2", unsafe.Sizeof(arr2))
	printSize("Slice1", unsafe.Sizeof(slice1))
	printSize("Slice2", unsafe.Sizeof(slice2))
	printSize("Slice3", unsafe.Sizeof(slice3))
}

func printSize(name string, size uintptr) {
	fmt.Printf("%10s's size : %d bytes\n", name, size)
}
